...
public class BuildWellStub extends BinaryWebSocketClient {

    private MPClient client;

    public BuildWellStub(MPClient client) throws IOException, WebSocketException {
        super("ws://mars-pioneer-322210.oa.r.appspot.com/api/action/buildWell");
        this.client = client;
    }
    ...
    public void handleResponse(BuildResponse response) {
        if (response.getStatus() == BuildResponse.Status.OK) {
            System.out.println("Water well built by " + client.getWorldSession().getId());
        } else {

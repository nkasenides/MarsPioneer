...
public class BuildMineWebSocket {
    ...
    public BuildMineWebSocket(HttpServletRequest request) {
    ...
    public void onMessage(byte[] bytes, int offset, int length) throws IOException {
        BuildMineRequest request = BuildMineRequest.parseFrom(bytes);
     ...
     */
    public void handleMessage(BuildMineRequest request) throws IOException {
        ...
        if (player.getSand() < BuildingType.MINE.getSandCost() ||
                player.getFood() < BuildingType.MINE.getFoodCost() ||
                player.getWater() < BuildingType.MINE.getWaterCost() ||
                player.getMetal() < BuildingType.MINE.getMetalCost()) {
            send(BuildResponse.newBuilder()
        ...
        //Can only build a mine on gravel or rock:
        if (cell.getType() != CellType.GRAVEL_CellType && cell.getType() != CellType.ROCK_CellType) {
            send(BuildResponse.newBuilder()
                    .setStatus(BuildResponse.Status.CANNOT_BUILD)
        ...
        //Deduct resources:
        player.setFood(player.getFood() - BuildingType.MINE.getFoodCost());
        player.setWater(player.getWater() - BuildingType.MINE.getWaterCost());
        player.setSand(player.getSand() - BuildingType.MINE.getSandCost());
        player.setMetal(player.getMetal() - BuildingType.MINE.getMetalCost());

        //Create building:
        BuildingEntity building = new BuildingEntity();
        building.setDirection(Direction4.NORTH);
        building.setBuildingType(EBuildingType.MINE_EBuildingType);
        building.setAreaOfInterest(BuildingType.MINE.getAreaOfInterest());
